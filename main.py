...
def generate_map(size):
    ...
    def generate_block(line, char):
        if (
            line == 0
            or line == bordered_diameter - 1
            or char == 0
            or char == bordered_diameter - 1
        ):
            return 3
        ...
        if (
            (
                line - 1 < size
                and char - 1 + (line - 1) >= size
                and char - 1 < size * 2 + (line - 1)
            )
            or (size <= line - 1 < size * 2)
            or (
                line - 1 >= size * 2
                and line - 1 - (char - 1) + 1 <= size * 2
                and char - 1 < size * 2 + diameter - (line - 1) - 1
            )
        ):
            return randint(1, 2)
        return 3
